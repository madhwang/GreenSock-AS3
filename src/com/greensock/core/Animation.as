...
public class Animation {
	...
	/* ActionScript 드로잉 API를 사용하여 가벼운 모양을 만드는 데 사용됩니다. */
	 ...
	 * 모든 프레임에서 새 인스턴스를 다시 만드는 것이 아니라 더 나은 메모리 관리를 위해이 이벤트 인스턴스를 재사용합니다
	 * **/
	 ...
	 * 가장 최근의 상위 타임 라인 (<code> _rootTimeline </ code> 및 <code> _rootFramesTimeline </ code>에 대한 <code> null </ code> 만 해당).
	 * 애니메이션이 부모 타임 라인에서 제거 될 때마다 <code> timeline </ code> 속성 ( "_"접두사 없음)이 null입니다.
	 * 우리는 약간 다른 방식으로 이것을 내부적으로 사용합니다.
	 * 마지막 부모 타임 라인에 대한 참조를 항상 유지해야 애니메이션을 다시 사용할 수 있도록 설정해야합니다. "_gc"는 TimelineLite가 완료 될 때와 같이
	 * Animation이 해당 타임 라인에서 제거되지 않고 gc에 적합 할 수 있다는 점에서 다릅니다.
	 * TimelineLite가 다시 시작되는 것처럼 필요한 경우 다시 활성화하는 것이 훨씬 빠릅니다.
	 * **/
	 ...
	 * 부모 타임 라인. 모든 애니메이션은 타임 라인 (기본적으로 루트 타임 라인)에 배치되며 하나의 부모 만 가질 수 있습니다. 인스턴스는 한 번에 여러 타임 라인에 존재할 수 없습니다.
	 * **/
	...
	public function Animation(duration:Number = 0, vars:Object = null) {
		...
		/*SimpleTimeline  의  add  메서드 */
		var tl:SimpleTimeline = (this.vars.useFrames) ? _rootFramesTimeline : _rootTimeline;
		/*  tl._time 은  Animation 의 멤버변수.  SimpleTimeline 은  Animation 의 자식 */
